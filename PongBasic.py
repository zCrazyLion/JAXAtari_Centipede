...
class PongGame(BaseGame):
    ...
    def get_oc_state(self):
        return jnp.array([[self.curr_state[4]],
                [self.curr_state[5]],
    ...
    @staticmethod
    @jax.jit
    def update_paddle(pos: float, movement: float, height: float, paddle_height: float) -> float:
    ...
    @staticmethod
    @jax.jit
    def update_cpu(curr_pos, ball_pos, height: float, paddle_height: float, time:float) -> float:
        new_pos = jnp.where(jnp.less(curr_pos-(paddle_height / 2), ball_pos[1]), curr_pos, curr_pos-1)
        new_pos = jnp.where(jnp.greater(curr_pos+(paddle_height / 2), ball_pos[1]), new_pos, new_pos+1)
    ...
    def check_paddle_collision(
        ...
        collision = jnp.logical_and(
            x_collision,
            jnp.logical_and(
                ball_pos[1] >= paddle_y_min,
                ball_pos[1] <= paddle_y_max
            )
        )
    ...
    def update_state(
        ...
        new_player_paddle_pos = PongGame.update_paddle(
            player_paddle_pos, p1_movement, height, paddle_height
        )
        ...
        cpu_paddle_collision, new_vel_after_cpu_paddle = PongGame.check_paddle_collision(
            new_ball_pos, ball_vel, new_cpu_paddle_pos, True,
            paddle_width, paddle_height, paddle_width, ball_size
        )

        player_paddle_collision, new_vel_after_player_paddle = PongGame.check_paddle_collision(
            new_ball_pos, new_vel_after_cpu_paddle, new_player_paddle_pos, False,
            width - paddle_width, paddle_height, paddle_width, ball_size
        )

        # Combine paddle collision velocities
        new_vel = jnp.where(cpu_paddle_collision | player_paddle_collision,
                           jnp.where(cpu_paddle_collision, new_vel_after_cpu_paddle, new_vel_after_player_paddle),
                           ball_vel)

        # Wall collisions
        wall_collision = jnp.logical_or(
            new_ball_pos[1] <= ball_size / 2,
            new_ball_pos[1] >= height - ball_size / 2
        )
